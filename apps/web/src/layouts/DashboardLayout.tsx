  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-[#020617] text-slate-900 dark:text-slate-50 transition-colors font-sans">
      ...
      <aside
        ...
        } bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 transition-all duration-300 fixed h-full z-40 flex flex-col`}
      >
        <div className="h-16 flex items-center px-6 border-b border-slate-200 dark:border-slate-800">
          <Link to="/" className="flex items-center gap-3 overflow-hidden group">
            <div className="bg-blue-600 p-2 rounded-lg shrink-0 group-hover:rotate-12 transition-transform">
              <Sparkles size={20} className="text-white" />
            </div>
            {sidebarOpen && (
              <span className="font-bold text-lg tracking-tight whitespace-nowrap text-slate-900 dark:text-white">
                Career AI
              </span>
            )}
          </Link>
          ...
          <div className="flex items-center gap-4">
            {user ? (
              <div className="flex items-center gap-3">
                ...
                <button 
                  onClick={() => navigate("/profile")}
                  className="h-9 w-9 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-400 flex items-center justify-center text-white font-bold text-sm shadow-sm hover:ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-slate-950 transition-all"
                >
                  {user.name.charAt(0).toUpperCase()}
                </button>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  to="/register"
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-all shadow-md shadow-blue-500/20"
                >
                  Get Started
                </Link>
              </div>
