...
export const useGenerateQuickCareerPath = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (role) => careerService.generateQuickCareerPath(role),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["career", "latest"] });
            queryClient.invalidateQueries({ queryKey: ["career", "paths"] });
        },
...
export const useLatestCareerPath = (enabled = true) => {
    return useQuery({
...
export const useCareerPaths = (enabled = true) => {
    return useQuery({
...
export const useCareerPathDetail = (id, enabled = true) => {
    return useQuery({
...
export const useUpdateMilestone = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ careerPathId, milestoneIndex, completed, }) => careerService.updateMilestone(careerPathId, milestoneIndex, completed),
...
export const useUpdateStatus = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ careerPathId, status, }) => careerService.updateStatus(careerPathId, status),
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: ["career", "paths"] });
            queryClient.invalidateQueries({ queryKey: ["career", "latest"] });
            queryClient.invalidateQueries({ queryKey: ["career", "detail", variables.careerPathId] });
...
export const useRefreshProgress = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ careerPathId, progress, }) => careerService.refreshProgress(careerPathId, progress),
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: ["career", "paths"] });
            queryClient.invalidateQueries({ queryKey: ["career", "latest"] });
            queryClient.invalidateQueries({ queryKey: ["career", "detail", variables.careerPathId] });
