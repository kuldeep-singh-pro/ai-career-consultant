...
const getCurrentRole = async (userId: string) => {
  ...
  return (
    latestResume?.currentRole ||
    latestResume?.inferredRole ||
...
export const generateCareerPathController =
asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (!skillgap)
    throw new Error("Skill gap required");

  const currentRole =
    await getCurrentRole(req.user._id);

  const skills =
    skillgap.currentSkills.map((s: any) =>
      typeof s === "string"
        ? s
        : s.name
    );
  ...
  const ai =
    await careerAi.generateCareerPathWithAi(
      currentRole,
      skillgap.targetRole,
      skills
    );
...
export const generateQuickCareerPathController =
asyncHandler(async (req: AuthRequest, res: Response) => {
...
asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const roadmap =
    await careerAi.generateRoadmapWithAi(
      currentRole,
      skillgap.targetRole,
      6
    );
...
export const getCareerPathsController =
asyncHandler(async (req: AuthRequest, res: Response) => {
...
export const getCareerPathsWithProgressController =
asyncHandler(async (req: AuthRequest, res: Response) => {

  const paths =
    await careerService
      .getCareerPathsWithProgress(req.user._id);

  return successResponse(res, "Fetched with progress", paths);
});


export const getLatestCareerPathController =
asyncHandler(async (req: AuthRequest, res: Response) => {
...
export const getCareerPathDetailController =
asyncHandler(async (req: AuthRequest, res: Response) => {
...
asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const path =
    await CareerPath.findById(req.params.id);
...
export const deleteCareerPathController =
asyncHandler(async (req: AuthRequest, res: Response) => {
